...
Object = "{A9CD2883-061D-11D4-B62B-00004C937F50}#1.0#0"; "CoCash111.ocx"
Object = "{C9E1AFB0-1172-11D7-83AD-0050DA238ADA}#1.0#0"; "Coptr111.ocx"
...
Begin VB.Form frmPOSMain 
   ...
   ClientWidth     =   12210
   FillColor       =   &H00E1E1E1&
   Icon            =   "frmPOSMain8_WithStore.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   7965
   ScaleWidth      =   12210
   ...
   Begin TrueOleDBGrid60.TDBGrid G1 
      Height          =   2925
      Left            =   60
      OleObjectBlob   =   "frmPOSMain8_WithStore.frx":038A
      TabIndex        =   0
      TabStop         =   0   'False
      Top             =   315
      Width           =   12000
   ...
   Begin VB.TextBox txtDiscounts 
      ...
      MultiLine       =   -1  'True
      TabIndex        =   12
      Text            =   "frmPOSMain8_WithStore.frx":5415
      Top             =   4245
      Visible         =   0   'False
      Width           =   2595
   End
   Begin VB.TextBox txtVouchers 
      ...
      MultiLine       =   -1  'True
      TabIndex        =   11
      Text            =   "frmPOSMain8_WithStore.frx":541B
      Top             =   4305
      Visible         =   0   'False
      Width           =   2595
   End
   ...
   Begin TrueOleDBGrid60.TDBGrid G4 
      Height          =   2070
      Left            =   30
      OleObjectBlob   =   "frmPOSMain8_WithStore.frx":5421
      TabIndex        =   4
      TabStop         =   0   'False
      Top             =   75
      Visible         =   0   'False
   ...
   Begin TrueOleDBGrid60.TDBGrid G2 
      Height          =   1245
      Left            =   60
      OleObjectBlob   =   "frmPOSMain8_WithStore.frx":AE30
      TabIndex        =   3
      TabStop         =   0   'False
      Top             =   3855
      Width           =   4485
   ...
   Begin TrueOleDBGrid60.TDBGrid G5 
      Height          =   2010
      Left            =   1950
      OleObjectBlob   =   "frmPOSMain8_WithStore.frx":E4A3
      TabIndex        =   13
      TabStop         =   0   'False
      Top             =   855
      Visible         =   0   'False
   ...
   Begin TrueOleDBGrid60.TDBGrid G3 
      Height          =   2400
      Left            =   1500
      OleObjectBlob   =   "frmPOSMain8_WithStore.frx":1270A
      TabIndex        =   14
      TabStop         =   0   'False
      Top             =   765
      Visible         =   0   'False
   ...
   Begin OposPOSPrinter_1_11_LibCtl.OPOSPOSPrinter OPOSPOSPrinterPosiflex 
      Left            =   11355
      OleObjectBlob   =   "frmPOSMain8_WithStore.frx":17C61
      Top             =   6510
   End
   Begin OposCashDrawer_1_11_LibCtl.OPOSCashDrawer OPOSCashDrawerPosiflex 
      Left            =   11370
      OleObjectBlob   =   "frmPOSMain8_WithStore.frx":17C85
      Top             =   6165
   End
...
10        On Error GoTo errHandler
      ...
      Dim i As Integer
        ...
        bTryToCOnnectToMainServer = True
...
410       End If
420       strPos = "3"
            If oPC.GetLoyaltyCode = "" Then
                oPC.dbConnectMain
                Set rst = New ADODB.Recordset
                rst.Open "SELECT CF_BC_ID,CF_LOYALTYCLUBTYPE FROM tCONFIGURATION", oPC.DBMainConn
                strSQL = "UPDATE tAPPSETTINGS SET BC_Code = '" & FNS(rst.Fields(0)) & "',Loyalty_Code = '" & FNS(rst.Fields(1)) & "'"
               oPC.OpenLocalDatabase
               oPC.DBLocalConn.Execute strSQL
                rst.Close
            End If
          bCollectRepcode = oPC.GetProperty("CollectRepCodeTF")
...
180                       MsgBox "The till printer (" & oPC.Printername & ") cannot be claimed by the application." & vbCrLf & "Result is " & CStr(lngResult) & ". This application will close."
...
'    txtPettyCash.Visible = False
    ...
    Select Case enPresentState
        ...
        Case eSale
            ...
            Else
                If oExchange.BalanceOwing < 0 Then
                    Stat "Scan or (X)Cancel trans.,(C)Cash refund,(CC)Reverse credit card,(CV)Issue credit voucher,(Dn)Del sale,(DPn)Del payment, (PS) Park sale ,(FC) Find customer"
                Else
                    Stat "Scan or (X)Cancel trans.,(C)Cash,(V)Voucher,(CC)Card,Q)Cheque,(AC)On account,(DDP) Direct deposit,(Dn)Del sale,(DPn)Del payment, (PS) Park sale,(FC) Find customer"
                End If
            End If
            SetForCOLSVisible False
            DisplayTotals
...
Dim i As Integer
    ...
    If MsgBox("Cancel this transaction?", vbQuestion + vbYesNo, "Confirm") = vbYes Then
        If CheckThisPoint(M_CLOSEAPPLICATION) Then
            ...
            Next i
'            For Each oPL In oExchange.PaymentLines
'                oPL.Delete
'            Next
            AcceptSale True
            oExchange.SetExchangeType eVoidAction
...
'    oExchange.SetExchangeType eVoidAction
End Sub
Private Sub AcceptSale(IsBeingCancelled As Boolean)
    On Error GoTo errHandler
10        On Error GoTo errHandler
      Dim lngRow As Long
...
30        If oExchange.NeedsCustomerInfo = True And IsBeingCancelled = False Then
...
1110                      Else
1120                          PrintSalesSlip 1, True, , True
1130                      End If
1140                  Else
...
40        oExchange.SupervisorID = oPC.ZSession.OpSession.OperatorID
...
Dim strDetails As String
    ...
    strDetails = ""
    Select Case Replace(oExchange.Customer.CustomerType, "*", "")
...
Dim i As Integer
    ...
    X1.InsertRows (0)
    X1.Value(1, 0) = X1.UpperBound(1)
    X1.Value(1, 1) = oSALELine.CodeF
    X1.Value(1, 2) = IIf(enPresentState = eSelectDepositLine, "(DEP)", "") & oSALELine.title & " (" & oSALELine.MainAuthor & ")"
    X1.Value(1, 3) = oSALELine.Qty
    X1.Value(1, 4) = oSALELine.PriceF & IIf(oSALELine.IsSpecialPrice, "**", "")
    X1.Value(1, 5) = oSALELine.DiscountRateF
    X1.Value(1, 6) = oSALELine.PLessDiscExtF
    If oExchange.transactionType <> "INV" Then
        X1.Value(1, 7) = oSALELine.PLessDiscExtVATF & "(" & oSALELine.VATRateF & ")"
    End If
...
Private Sub PrintSalesSlip(pCopyCount As Integer, Optional bReprint As Boolean, Optional toA4 As Boolean = False, Optional bBeingVoided As Boolean)
    On Error GoTo errHandler
Dim i As Integer
Dim c As Integer
...
strErrPos = "2"
                    With OPOSPrinter
                        PrintHeader ConvertToType(oExchange.transactionType), OPOSPrinter, bReprint, bBeingVoided     'Print header
strErrPos = "3"
...
Private Sub PrintHeader(eDocumentType As enumDocumentType, pPrinter As Object, Optional bReprint As Boolean, Optional bBeingVoided As Boolean)
    On Error GoTo errHandler
Dim fDate As String
Dim ar() As String
Dim i As Integer
...
'================================================================================================================================
    ...
    strExchangeMsg = Replace(strExchangeMsg, "|", "/") & "|"
    For i = 1 To oExchange.SaleLines.Count
        ...
        strExchangeMsg = strExchangeMsg & Replace(strCSLPart, "|", "/") & IIf(i = oExchange.SaleLines.Count, "", "~")
    Next i
    ...
    For i = 1 To oExchange.PaymentLines.Count
        ...
        strExchangeMsg = strExchangeMsg & Replace(strPayPart, "|", "/") & IIf(i = oExchange.PaymentLines.Count, "", "~")
    Next i
...
Private Function DetermineReturnToState() As eState
    ...
    Case emode_Sale
        ...
        Else
            DetermineReturnToState = eConfirmation
        End If
    End Select
    Exit Function
errHandler:
    ...
    If IsNumeric(iRow) Then
        ...
        If lngRow <= oExchange.SaleLines.Count And lngRow > 0 Then
            If CheckThisPoint(M_DELETELINE) Then
                If SecurityControl(enSECURITY_ISSUPERVISOR, lngSupervisorID, strName, bCancelled, "Enter security code", "You are not entitled to remove a sale line.") = True Then
