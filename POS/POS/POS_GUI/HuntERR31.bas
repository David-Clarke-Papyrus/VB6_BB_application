...
'comment/uncomment lines in ErrSaveToPC method.
'
'Public members =========================================================================
'
...
'Public Function ErrSaveToPC(ByVal ConnectString As String, _
...
'Define your custom errors starting with MAP_APP_FIRST + 1.
#If H_NOENUMS = 1 Then
...
Public Enum ENUM_ERRMAP
    ...
    MAP_FIRST = MAP_BASE
    MAP_RESERVED_FIRST = MAP_FIRST            'Errors reserved for HuntERR and UJ apps.
    ...
    MAP_EXC_FIRST = MAP_RESERVED_LAST + 1     'Exceptions - reraised by ErrorIn
        ...
        EXC_NOSERVERRS = MAP_EXC_FIRST + 3
...
Public Function ErrorIn(ByVal MethodHeader As String, _
    ...
    mExtBuffer = ""
    ErrRestore
    mPreserved = False 'Drop the flag, as we made use of err info
...
Public Sub ErrPreserve()
    If (Err.Number <> 0) And (Not mPreserved) Then
        ...
        mErl = Erl
        mAPI = Err.LastDllError 'We need to do it here, GetLastError information is vulnerable
        mPreserved = True
    End If
End Sub
...
Public Function IsException(ByVal AnErrNumber As Long) As Boolean
    IsException = (AnErrNumber >= MAP_EXC_FIRST) And (AnErrNumber <= MAP_EXC_LAST)
End Function
...
Public Property Get ErrDescription() As String
    ErrDescription = mDescr
End Property

Public Property Get ErrHelpContext() As String
    ErrHelpContext = mHelpContext
End Property

Public Property Get ErrHelpFile() As String
    ErrHelpFile = mHelpFile
End Property

'Extracts original error source from error report
Public Property Get ErrOrigSource() As String
    ErrOrigSource = IIf(InPropagation, ErrExtractFromReport(vbNewLine & "  Source: ", vbNewLine), mSource)
End Property
...
Public Function ErrMustStop() As Boolean
    ...
    ErrPreserve
    If ErrInIDE And (Not InException) And ErrStopFlag And mHelpFile <> STR_NOSTOP Then
        Debug.Print
...
Public Function ErrSaveToFile(Optional ByVal ErrFileName As String = "Errors.txt") As Boolean
    ...
    If ErrReport <> "" Then
        f = FreeFile
        fName = IIf(InStr(1, ErrFileName, "\") > 0, ErrFileName, oPC.SharedFolderRoot & "\" & ErrFileName)
...
Public Function LogSaveToFile(pMsg As String, Optional ByVal ErrFileName As String = "Errors_POS.txt") As Boolean
    Dim f As Long, fName As String
    On Error GoTo errHandler
        f = FreeFile
        fName = IIf(InStr(1, ErrFileName, "\") > 0, ErrFileName, oPC.SharedFolderRoot & "\" & ErrFileName)
...
errHandler:
    'nothing to do...
End Function
Public Function LogSaveToFileLocal(pMsg As String, Optional ByVal ErrFileName As String = "Errors_POS.txt") As Boolean
    Dim f As Long, fName As String
    On Error GoTo errHandler
        f = FreeFile
        fName = IIf(InStr(1, ErrFileName, "\") > 0, ErrFileName, oPC.LocalRootFolder & "\" & ErrFileName)
        If ErrOpenErrorFile(fName, f) Then
            Print #f, CStr(Now()) & "   " & pMsg & vbNewLine & vbNewLine
            Close #f
        End If
    LogSaveToFileLocal = True
    Exit Function
errHandler:
    'nothing to do...
End Function
...
Const TEMPLATE_REPORT = _
  ...
  "  Number: %ErrNum% = &H%ErrHex% = vbObjectError %ErrNumRel1% = MAP_APP_FIRST %ErrNumRel2% %ErrStd%%nl%" & _
  "  Source: %Source% %nl%" & _
  "  Description: %Descr%%nl%"
    ...
    ReportSet "ErrNum", mNumber
    ReportSet "ErrHex", Hex$(mNumber)
    ReportSet "ErrNumRel1", FormatNum(mNumber - vbObjectError)
    ReportSet "ErrNumRel2", FormatNum(mNumber - MAP_APP_FIRST)
    ReportSet "ErrStd", IIf(mNumber = ERR_GENERAL, "= ERR_GENERAL", "")
...
Private Sub ReportAddAPIError()
    ...
    If (mAPI <> 0) And (mAPI <> 203) Then
        ReportAdd "  API Error: (" & mAPI & ") " & FormatMessage(mAPI)
    End If
    Exit Sub
errHandler:
    'Nothing to do...
...
Public Sub GetFromServer(ByVal Extractor, ByVal COMServer As Object, _
    ...
    ErrPreserve
    On Error GoTo errHandler
    sHdr = "    COM Server Errors: Server=" & VarToString(COMServer) & _
        " Extractor=" & VarToString(Extractor) & IIf(Comment = "", "", "  [" & Comment & "]")
    If IsObject(Extractor) Then Set objExtr = Extractor Else Set objExtr = CreateObject(CStr(Extractor))
    Restore 'Set  object with original Error information, in case if extractor needs it
    sMsg = objExtr.Extract(COMServer, Param)
    If sMsg <> "" Then
        sMsg = Unindent(Trim$(sMsg))
        If Right$(sMsg, Len(vbNewLine)) = vbNewLine Then sMsg = Left$(sMsg, Len(sMsg) - 1) 'cut off new line char
...
errHandler:
    mExtBuffer = mExtBuffer & sHdr & vbNewLine & _
        "    ErrorIn failed to extract Error information: " & Err.Description
ExitSub:
    Restore 'Restore  object for code in Error ErrHandler
End Sub
