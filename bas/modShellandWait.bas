...
Public Function ShellAndWaitb(ShellCommand As String, _
                    TimeOutMs As Long, _
...
If Trim(ShellCommand) = vbNullString Then
    ShellAndWaitb = ShellAndWaitResult.InvalidParameter
    Exit Function
End If
...
If (Err.Number <> 0) Or (TaskID = 0) Then
    ShellAndWaitb = ShellAndWaitResult.Failure
    Exit Function
End If
...
Do Until WaitRes = WAIT_OBJECT_0
    ...
    Select Case WaitRes
        Case WAIT_ABANDONED
            ...
            ShellAndWaitb = ShellAndWaitResult.SysWaitAbandoned
        Case WAIT_OBJECT_0
            ' Successful completion
            ShellAndWaitb = ShellAndWaitResult.Success
        Case WAIT_FAILED
            ' attach failed
            ShellAndWaitb = ShellAndWaitResult.Success
        Case WAIT_TIMEOUT
            ...
            If Ms > 0 Then
                ...
                If ElapsedTime > Ms Then
                    ShellAndWaitb = ShellAndWaitResult.Timeout
                    Exit Do
                Else
...
If Err.Number = ERR_BREAK_KEY Then
    If BreakKey = ActionOnBreak.AbandonWait Then
        CloseHandle ProcHandle
        ShellAndWaitb = ShellAndWaitResult.UserWaitAbandoned
        Application.EnableCancelKey = SaveCancelKey
        Exit Function
    ...
    ElseIf BreakKey = ActionOnBreak.PromptUser Then
        ...
        If MsgRes = vbNo Then
            CloseHandle ProcHandle
            ShellAndWaitb = ShellAndWaitResult.UserBreak
            Application.EnableCancelKey = SaveCancelKey
        Else
            Err.Clear
            Resume Next
