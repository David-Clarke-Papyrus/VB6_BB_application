...
Public Function ErrSaveToFile(Optional ByVal ErrFileName As String = "Errors.txt") As Boolean
    ...
    If ErrReport <> "" Then
        f = FreeFile
        fName = IIf(InStr(1, ErrFileName, "\") > 0, ErrFileName, strServerMachineSharedFolder & "\" & ErrFileName)
...
Public Function LogSaveToFile(pMsg As String, Optional ByVal FileName As String = "Errors.txt") As Boolean
    ...
    On Error GoTo errHandler
        f = FreeFile
        fName = IIf(InStr(1, FileName, "\") > 0, FileName, strServerMachineSharedFolder & "\" & FileName)
