...
Public Function TryToDeleteFile(pFn As String) As Boolean
Dim fs As New FileSystemObject
Dim Res As Boolean

    TryToDeleteFile = True
    On Error Resume Next
    If fs.FileExists(pFn) Then
        fs.DeleteFile (pFn)
        If Err Then
            MsgBox "The application is trying to create a file called " & pFn & " and this is the name of a file already open on this computer." & vbCrLf & "Close the application using the file and try again.", vbInformation + vbOKOnly, "Can't do this"
            TryToDeleteFile = False
        End If
    End If
