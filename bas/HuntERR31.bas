...
'Public Function SaveToFile(Optional ByVal FileName As String = "Errors.txt") As Boolean
...
Public Enum ENUM_MAP
        ...
        MAP_FIRST = MAP_BASE
        MAP_RESERVED_FIRST = MAP_FIRST            'Errors reserved for Hunt and UJ apps.
...
Private mExtBuffer As String     'Errors from COMServers, added by GetFromServer
...
Public Function ErrorIn(ByVal MethodHeader As String, _
    ...
    If (Not mPreserved) And (Err.Number = 0) Then
        ...
        Err.Description = "ErrorIn: Error information was lost. " & _
            "To fix: call ErrPreserve before doing anything in Error ErrHandler."
    End If
    ErrPreserve
...
'Returns Error source saved by last call to ErrPreserve or ErrorIn, Error curre
Public Property Get Source() As String
    Source = mSource
End Property

'Returns Error description saved by last call to ErrPreserve Error ErrorIn, Error curre
Public Property Get Description() As String
    Description = mDescr
End Property
...
Public Function ErrMustStop() As Boolean
    ...
    If ErrInIDE And (Not InException) And StopFlag And mHelpFile <> STR_NOSTOP Then
        Debug.Print
        Debug.Print Format(Now, "hh:nn:ss") & " Stopped on Error:" & vbNewLine & Description
        Select Case MsgBox(StopPrompt, vbYesNoCancel Or vbCritical, "Stopped on Error")
...
Public Function SaveToPC(ByVal ConnectString As String, _
    ...
    If ErrReport <> "" Then
        'SQL Server:
        SQL = "Exec " & ProcName & " " & AppID & ", " & "'" & Replace(ErrReport, "'", "''") & "','" & GetComputerName & "','" & App.EXEName & "','" & GetAppVersion & "'"
        'Oracle:
...
Private Function SafeCallMethod(ByVal obj As Object, ByVal Method As String) As String
    ...
    End Select
    SafeCallMethod = IIf(Err.Number = 0, "succeeded", "failed with Error '" & Err.Description & "'")
End Function
