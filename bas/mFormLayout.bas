...
Dim sDefault As String
    ...
    For i = 0 To CM.Splitters.Count - 1
        sDefault = CLng(GetSetting("PBKS", f.Name & CStr(i), "Dims", "0"))
        If sDefault <> "0" Then
            CM.MoveSplitter i, CLng(sDefault)
        End If
    Next
    Exit Sub
errHandler:
...
Dim i As Integer
    For i = 1 To pG.ColumnHeaders.Count
        pG.ColumnHeaders(i).Width = GetSetting("PBKS", pFormName, CStr(i), CInt(pG.ColumnHeaders(i).Width))
    Next
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
...
Sub LoadContourcubeLayout(ltFile As String, pCC As ContourCubeX)
    On Error Resume Next
...
Sub ExpandSection(strAxis As String, strExpand As String, pCC As ContourCubeX)
    On Error Resume Next
