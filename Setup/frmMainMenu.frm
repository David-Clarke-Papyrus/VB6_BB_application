...
Dim strCmd As String
   ...
   ' Autoinstall
    If optServer = True Then
        strCmd = "setup.exe"
        F_7_AB_1_ShellAndWaitSimple strCmd, vbNormalFocus
       ' cmdServer_Install
    Else
        If optWorkstation = True Then
            strCmd = "PBKS Install(WS-NOPOS).exe"
            F_7_AB_1_ShellAndWaitSimple strCmd, vbNormalFocus
        Else
            If optTill = True Then
                strCmd = "PBKS Install(POS).exe"
                F_7_AB_1_ShellAndWaitSimple strCmd, vbNormalFocus
            End If
        End If
    End If
...
'
'
''    oMF.CreateFolder Left(Drive1.Drive, 1) & ":\PBKS", True, "PBKS_S"
...
'    With oShortCut
'        .TargetPath = strRoot & "\Executables\Dispatch.EXE"
