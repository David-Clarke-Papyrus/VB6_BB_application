...
Public Enum ENUM_ERRMAP
    ...
    ERRMAP_EXC_FIRST = ERRMAP_RESERVED_LAST + 1     'Exceptions - reraised by ErrorIn
        ...
        EXC_NOCONNECTION = ERRMAP_EXC_FIRST + 1
        EXC_NOSERVER = ERRMAP_EXC_FIRST + 2
        EXC_NOSERVERRS = ERRMAP_EXC_FIRST + 3
        EXC_MASTERDATASET = ERRMAP_EXC_FIRST + 4
        EXC_MISSING_RS = ERRMAP_EXC_FIRST + 5
        EXC_INVALIDFOLDERS = ERRMAP_EXC_FIRST + 6
        EXC_SERVERUNAVAILABLE = ERRMAP_EXC_FIRST + 7
        EXC_RECORDLOCKED = ERRMAP_EXC_FIRST + 8
        EXC_DUPLICATERECORD = ERRMAP_EXC_FIRST + 9
    ERRMAP_EXC_LAST = ERRMAP_EXC_FIRST + 2000
    ERRMAP_APP_FIRST
...
Const MAX_RETRY = 1# / 24# / 60# / 60#  ' 2000 ms
...
Public Function ErrSaveToFile(Optional ByVal ErrFileName As String = "Errors.txt") As Boolean
    ...
    If ErrReport <> "" Then
        f = FreeFile
      '  FName = IIf(InStr(1, ErrFileName, "\") > 0, ErrFileName, strSharedServerFolder & "\" & ErrFileName)
        FName = IIf(InStr(1, ErrFileName, "\") > 0, ErrFileName, gLocalRootFolder & "\" & ErrFileName)
...
errHandler:
    'nothing to do...
End Function
Public Function LogSaveToFile(pMsg As String, Optional ByVal ErrFileName As String = "Errors.txt") As Boolean
    Dim f As Long, FName As String
    On Error GoTo errHandler
        f = FreeFile
        FName = IIf(InStr(1, ErrFileName, "\") > 0, ErrFileName, gLocalRootFolder & "\" & ErrFileName)
...
Const FORMAT_MESSAGE_FROM_SYSTEM = &H2000
