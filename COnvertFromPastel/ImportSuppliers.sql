...
--Insert suppliers details to tTP
INSERT INTO tTP (TP_NAME,TP_ROLE,TP_INITIALS,TP_TITLE,TP_PHONE,TP_VATABLE,TP_VATNUMBER,TP_ACNO,TP_CELL,OLDTPID)
SELECT DESCR,2,'','',tel,CASE WHEN currcode <> 0 THEN 0 ELSE 1 END,'',NUMR,Mobile,ID FROM __SupplierAccounts
...
SELECT TP_ID,ADD1,ADD2,ADD3,ADd4,ADD5,'',Tel,fax,email,CASE WHEN CONTACT > '' THEN CONTACT ELSE Descr END,1,0,1,1,ID 
		FROM __SupplierAccounts JOIN tTP ON OLDTPID = ID WHERE TP_ROLE = 2

--Insert second supplier's address
INSERT INTO tADD (ADD_TP_ID,ADD_L1,ADD_L2,ADD_L3,ADD_L4,ADD_ADDRESSEE,ADD_ORDERTo,ADD_DELTO,ADD_BILLTO,ADD_DEFAULT, OLDID)
SELECT TP_ID,delADD1,delADD2,delADD3,delADd4,CASE WHEN CONTACT > '' THEN CONTACT ELSE Descr END,0,1,0,0,ID 
		FROM __SupplierAccounts JOIN tTP ON OLDTPID = ID WHERE TP_ROLE = 2
