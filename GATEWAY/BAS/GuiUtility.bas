...
'End Sub
Public Sub HandleErrorQuiet(pCLose As Boolean, pbEmail As Boolean)
'Dim oEMail As z_EMail
...
'
'        Set oEMail = New z_EMail
'        oEMail.InitializeMail
'        oEMail.SendMailA "Sales data not sent: " & Format(Date, "dddd d mmmm yyyy"), "Sales data not sent", oPC.AdministratorEmail
'        oEMail.TerminateMail
'        Set oEMail = Nothing
'
'        fInet.Hangup
    End If
    ...
    Exit Sub
errHandler:
    MsgBox "URGENT: Nielsen sales not sent - talk to support person"
End Sub
