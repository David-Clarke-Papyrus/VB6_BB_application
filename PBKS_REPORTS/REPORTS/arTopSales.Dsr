...
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} arTopSales 
   ...
   _ExtentX        =   21299
   _ExtentY        =   14526
   SectionData     =   "arTopSales.dsx":0000
End
...
Public Sub Component(pRS As ADODB.Recordset, pDate1 As Date, pDate2 As Date)
    ...
    lblRptHeader.Caption = "Cash Sales generated between " & Format(pDate1, "dd/mm/yyyy") _
                                        & " and " & Format(pDate2, "dd/mm/yyyy")
    Me.lblFooter.Caption = "Cash Sales"
...
Dim cTmp2 As Currency
    ...
    cTmp1 = FNC(rs!CSL_Price) * FNN(rs!CSL_Qty) / 100
    txtPrice.Text = Format(cTmp1, "Standard")
    cTmp2 = (FNN(rs!CSL_Qty) * FNC(rs!CSL_Price)) * (1 - FNDBL(rs!CSL_Discount)) / 100
