//------------------------------------------------------------------------------
// Custom SQL StoredProcedure Profile Provider
//------------------------------------------------------------------------------

#region Custom SQL StoredProcedure Profile Provider
...
namespace CustomProfile
{
    ...
    public class SqlStoredProcedureProfileProvider : ProfileProvider
    {
        ...
        private string readSproc;
        private string setSproc;
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            sqlConnectionString = GetConnectionString(temp);
            ...
            setSproc = config["setProcedure"];
            if (String.IsNullOrEmpty(setSproc))
            {
                throw new ProviderException("setProcedure not specified");
            }

            readSproc = config["readProcedure"];
            if (String.IsNullOrEmpty(readSproc))
            {
        ...
        #region get default application name
        ...
        #region get/set application name
        ...
        #region get profile data from stored procedures
        ...
        private void GetProfileDataFromSproc(SettingsPropertyCollection properties, SettingsPropertyValueCollection svc, string username, SqlConnection conn, bool userIsAuthenticated)
        {
            ...
            SqlCommand cmd = CreateSprocSqlCommand(readSproc, conn, username, userIsAuthenticated);
            try
            {
        ...
        #region Container struct for use in aggregating columns for queries
        ...
        #region Helper that just sets up the usual sproc sqlcommand parameters and adds the applicationname/username
        ...
        public override void SetPropertyValues(SettingsContext context, SettingsPropertyValueCollection collection)
        {
            ...
            try
            {
                ...
                foreach (SettingsPropertyValue pp in collection)
                {
                    ...
                    // Unlike the table provider, the sproc provider works against a fixed stored procedure signature, and must provide values for each stored procedure parameter
                    // if (!pp.IsDirty && pp.UsingDefaultValue) // Not fetched from DB and not written to continue;
                ...
                cmd = CreateSprocSqlCommand(setSproc, conn, username, userIsAuthenticated);
