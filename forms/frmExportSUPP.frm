...
Begin VB.Form frmExportSUPP 
   BackColor       =   &H00D3D3CB&
   Caption         =   "Export data from suppliers"
   ClientHeight    =   5790
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   8160
   ...
   Begin TrueOleDBGrid60.TDBGrid G1 
      Height          =   4515
      Left            =   105
      OleObjectBlob   =   "frmExportSUPP.frx":0000
      TabIndex        =   2
      Top             =   555
      Width           =   7395
   End
   ...
   End
End
Attribute VB_Name = "frmExportSUPP"
...
Dim bMore As Boolean
    ...
    If FetchData(CLng(txtMax), bMore) Then
        LoadGrid
        frmTP.component XA, XB, "SUPPLIER"
        frmTP.Show vbModal
    End If
...
Dim strWhere As String
    ...
    strSQL = "SELECT " & strFields & " FROM tTP JOIN tADD on ADD_TP_ID = TP_ID  " & strWhere
...
Dim lngIndex As Long
    ...
    rs.Open "Select * from tDmp WHERE Type = 'S'", oPC.COShort, adOpenKeyset, adLockOptimistic
