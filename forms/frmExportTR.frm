...
Begin VB.Form frmExportTR 
   BackColor       =   &H00D3D3CB&
   Caption         =   "Export data from documents"
   ClientHeight    =   5790
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   8160
   ...
   Begin TrueOleDBGrid60.TDBGrid G1 
      Height          =   4680
      Left            =   105
      OleObjectBlob   =   "frmExportTR.frx":0000
      TabIndex        =   2
      Top             =   390
      Width           =   7395
   End
...
Dim frm As New frmProdTemplate
Dim lngFldCount As Long
Dim lngIndex As Long
Dim enExportType As enumExportType

Public Sub component(pType As enumExportType)
    enExportType = pType
End Sub
...
Dim i As Long
   For i = 1 To frm.GT.Splits(0).Columns.Count
        frm.GT.Splits(0).Columns.Remove (0)
   Next
    For i = XA.UpperBound(1) To 1 Step -1
        If XA(i, 3) = True Then
            Set Col = frm.GT.Splits(0).Columns.Add(0)
            Col.Caption = XA(i, 2)
            Col.Visible = True
        End If
    Next i
...
Dim strWhere As String
    ...
    strWhere = Replace(UCase(strWhere), """", "'")
 '   If right(strWhere, 2) = "*'" Then
 '       strWhere = "Like " & strWhere
 '   End If
    If enExportType = enDocument Then
        strSQL = "SELECT " & strFields & " FROM tTR " _
            & " LEFT JOIN tInvoice on TR_ID = INV_ID LEFT JOIN tILine ON IL_TR_ID = TR_ID " _
            & " LEFT JOIN tPO on TR_ID = PO_ID LEFT JOIN tPOL ON POL_tr_ID = TR_ID " _
            & " LEFT JOIN tDEAL on P_DEALID = DL_ID" '& strWhere & " AND TR_STATUS > 2"
    Else
        strSQL = "SELECT " & strFields & " FROM tProduct LEFT JOIN tPT on P_ProductType_ID = PT_ID LEFT JOIN tTP on P_SupplierID = TP_ID LEFT JOIN tDEAL on P_DEALID = DL_ID" & strWhere
    End If
...
Dim lngIndex As Long
    ...
    rs.Open "Select * from tDmp WHERE Type = 'P'", oPC.CO, adOpenKeyset, adLockOptimistic
...
Private Sub G1_AfterUpdate()
Dim i As Integer
Dim bm
    bm = G1.Bookmark
    For i = 1 To bm - 1
        If XA(i, 1) >= CInt(XA(bm, 1)) Then
            XA(i, 1) = XA(i, 1) + 1
        End If
    Next
    XA.QuickSort 1, XA.UpperBound(1), 1, XORDER_ASCEND, XTYPE_LONG  ', 4, XORDER_ASCEND, XTYPE_DATE
