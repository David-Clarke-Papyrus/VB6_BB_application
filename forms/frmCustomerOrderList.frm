...
Begin VB.Form frmCustomerOrderList 
   ...
   Begin VB.CommandButton cmdCreateInvoices 
      BackColor       =   &H00C4BCA4&
      Caption         =   "Create invoices for selected school and class"
      Height          =   345
      Left            =   5520
      Style           =   1  'Graphical
      TabIndex        =   10
   ...
   Begin VB.CommandButton cmdGenerate 
      BackColor       =   &H00C4BCA4&
      Caption         =   "Prepare for invoicing (generates lines for the selected school and class)"
      Height          =   345
      Left            =   1650
      Style           =   1  'Graphical
      TabIndex        =   4
   ...
   Begin TrueOleDBGrid60.TDBGrid G 
      Bindings        =   "frmCustomerOrderList.frx":0000
      Height          =   2160
      Left            =   210
      OleObjectBlob   =   "frmCustomerOrderList.frx":0015
      TabIndex        =   0
      Top             =   2310
      Width           =   8730
   End
   ...
   End
End
Attribute VB_Name = "frmCustomerOrderList"
...
Private Sub cmdCreateInvoices_Click()
    If MsgBox("You want to create invoices for " & cboSchoolCode & ": " & cboClassCode & "?", vbQuestion + vbYesNo, "Confirm") = vbNo Then
        Exit Sub
    End If
    Screen.MousePointer = vbHourglass
    CreateInvoices cboSchoolCode, cboClassCode
    Screen.MousePointer = vbDefault
    
    MsgBox "Invoices created", vbInformation, "Status"
End Sub
...
Private Sub cmdGenerate_Click()
    ...
    '
    oPC.OpenDBSHort
    oPC.COShort.Execute "INSERT INTO tTmpInvoiceLine (TIL_SC_ID,TIL_PLI_ID,TIL_QTY) SELECT SC_ID,PLI_ID,1 FROM vSchoolsInvoiceGeneration WHERE SC_SCHOOLCODE = '" & Me.cboSchoolCode & "' AND SC_CLASSCODE = '" & Me.cboClassCode & "'"
...
errHandler:
    ...
    If Err = -2147217873 Then
        MsgBox "This class has had lines generated already.", vbInformation, "Can't do this"
        Exit Sub
    End If
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmCustomerOrderList.cmdGenerate_Click"
End Sub
...
Private Sub LoadCustomerLists()
    Adodc1.CommandType = adCmdText
    Adodc1.RecordSource = "Select * FROM vCustomerOrderLines WHERE SC_SCHOOLCODE = '" & cboSchoolCode & "' AND SC_CLASSCODE = '" & cboClassCode & "' AND SC_NAME = '" & Me.cboCustomers & "'"
...
Private Sub LoadSchools()
    ...
    Adodc2.RecordSource = "Select PLI_LISTNAME FROM tProductList GROUP BY PLI_LISTNAME ORDER BY PLI_LISTNAME"
...
Private Sub LoadGrades()
    ...
    With Adodc3.Recordset
        ...
        Loop
    End With
    If cboClassCode.ListCount > 0 Then cboClassCode = cboClassCode.List(0)
    
End Sub
Private Sub LoadCustomers()
    Adodc4.CommandType = adCmdText
    Adodc4.RecordSource = "Select SC_NAME FROM tSchoolsCustomer WHERE SC_SCHOOLCODE = '" & cboSchoolCode & "' AND SC_CLASSCODE = '" & cboClassCode & "'GROUP BY SC_SCHOOLCODE,SC_CLASSCODE,SC_NAME ORDER BY SC_NAME"
    Adodc4.ConnectionString = oPC.ConnectionString
    Adodc4.Refresh
    
    cboCustomers.Clear
    With Adodc4.Recordset
        Do While Not .EOF
            cboCustomers.AddItem .Fields(0)
            .MoveNext
        Loop
    End With
    If cboCustomers.ListCount > 0 Then cboCustomers = cboCustomers.List(0)
    LoadCustomerLists
End Sub
...
Private Sub CreateInvoices(pSchoolCOde As String, pClassCode As String)
Dim iresult As Long
...
'-------------------------------
    ...
    cmd.CommandText = "CreateSchoolsInvoices"
    cmd.CommandType = adCmdStoredProc
    
    Set par = cmd.CreateParameter("@SchoolCode", adVarChar, adParamInput, 50, pSchoolCOde)
    cmd.Parameters.Append par
    Set par = cmd.CreateParameter("@ClassCode", adVarChar, adParamInput, 50, pClassCode)
    cmd.Parameters.Append par
    Set par = cmd.CreateParameter("@TRSTATUS", adInteger, adParamInput, , 2)
    cmd.Parameters.Append par
    Set par = cmd.CreateParameter("@TRDATE", adDate, adParamInput, , Date)
    cmd.Parameters.Append par
    Set par = cmd.CreateParameter("@TPID", adInteger, adParamInput, , oPC.Configuration.CSCustomerID)
    cmd.Parameters.Append par
