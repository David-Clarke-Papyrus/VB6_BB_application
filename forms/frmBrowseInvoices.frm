...
Private Sub Grid_MouseDown(Button As Integer, Shift As Integer, x As Single, Y As Single)
   ...
   If Button = 2 Then   ' Check if right mouse button
                        ' was clicked.
      PopupMenu Forms(0).mnuBrowseInvoicesPopup   ' Display the File menu as a
                        ' pop-up menu.
   End If
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmBrowseInvoices.Grid_MouseDown(Button,Shift,x,Y)", Array(Button, Shift, x, Y), _
         EA_NORERAISE
    HandleError
End Sub
...
Dim arPL As New arInvoicePickListCombined
Dim rs As ADODB.Recordset
    ...
    For i = 0 To Grid.SelBookmarks.Count - 1
        IDset = IDset & Replace(CStr(XA(Grid.SelBookmarks(i), 6)), "K", "") & ","
...
'-------------------------------
        ...
        rs.Open "SELECT DestinationAcno,DestinationName, IL_Price as Price,WOH, P_Title,P_MainAuthor, CODEF,IL_Ref,SUM(IL_Qty) as IL_Qty,dbo.FlattenSections(IL_P_ID) as bin FROm vzInvoiceLine_Fetch " _
                & " WHERE IL_TR_ID in (" & IDset & ") " _
                & " Group BY DestinationAcno,DestinationName,IL_Price,WOH,P_Title,P_MainAuthor,CODEF,IL_Ref,dbo.FlattenSections(IL_P_ID) " _
                & " ORDER BY BIN,DestinationName,P_TITLE,CODEF, IL_Qty ", oPC.COShort, adOpenForwardOnly
        
        arPL.component rs, Doccodes
        arPL.Show vbModal
        
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmBrowseInvoices.mnuBrowseInvoicesPopup_Print_Click"
End Sub
...
Dim i As Integer
    ...
    For i = 1 To cInv.Count
        With objItem
            ...
            XA.Value(i, 2) = cInv(i).Ref & cInv(i).StaffNameB & " " & cInv(i).Exchange
