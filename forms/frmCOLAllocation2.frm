...
Begin VB.Form frmCOLAllocation 
   ...
   Begin VB.CommandButton cmdExcelExport 
      ...
      Height          =   615
      Left            =   5730
      Picture         =   "frmCOLAllocation2.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   12
   ...
   Begin VB.Frame frmGenerate 
      ...
      Begin VB.CommandButton cmdGenGDNs 
         ...
         Height          =   870
         Left            =   480
         Picture         =   "frmCOLAllocation2.frx":038A
         Style           =   1  'Graphical
         TabIndex        =   23
      ...
      Begin VB.CommandButton cmdGenAppros 
         ...
         Height          =   870
         Left            =   465
         Picture         =   "frmCOLAllocation2.frx":0714
         Style           =   1  'Graphical
         TabIndex        =   22
         ToolTipText     =   "Creates invoices for all products/customer orderlines where qty > 0"
         Top             =   1170
      ...
      Begin VB.CommandButton cmdGenInv 
         ...
         Height          =   870
         Left            =   465
         Picture         =   "frmCOLAllocation2.frx":0A9E
         Style           =   1  'Graphical
         TabIndex        =   11
         ToolTipText     =   "Creates invoices for all products/customer orderlines where qty > 0"
         Top             =   1170
   ...
   Begin VB.CommandButton cmdPrint 
      ...
      Height          =   615
      Left            =   6750
      Picture         =   "frmCOLAllocation2.frx":0E28
      Style           =   1  'Graphical
      TabIndex        =   6
      ToolTipText     =   "Sets all the allocations to those shown when the form was opened"
      Top             =   5415
   ...
   Begin VB.CommandButton cmdQuit 
      ...
      Height          =   615
      Left            =   7770
      Picture         =   "frmCOLAllocation2.frx":11B2
      Style           =   1  'Graphical
      TabIndex        =   2
   ...
   Begin VB.CommandButton cmdSave 
      ...
      Height          =   615
      Left            =   8790
      Picture         =   "frmCOLAllocation2.frx":153C
      Style           =   1  'Graphical
      TabIndex        =   1
   ...
   Begin TrueOleDBGrid60.TDBGrid Grid1 
      Height          =   4590
      Left            =   165
      OleObjectBlob   =   "frmCOLAllocation2.frx":18C6
      TabIndex        =   0
      Top             =   90
      Width           =   9615
   End
...
Dim strWSName As String
    ...
    For Each oALLOC In cALLOC
        ...
        XA.Value(lngIndex, 18) = oALLOC.PID
        ...
        XA.Value(lngIndex, 4) = oALLOC.CreateInvoice
        XA.Value(lngIndex, 5) = oALLOC.OrderCode
        XA.Value(lngIndex, 6) = oALLOC.OrderDateF
        ...
        If oPC.AllowsSSInvoicing Then
            XA.Value(lngIndex, 7) = oALLOC.OrderedQty & "/" & oALLOC.OrderedSSQTY
            XA.Value(lngIndex, 8) = oALLOC.DeliveredSoFarFirm & "/" & oALLOC.DeliveredSoFarSS
            XA.Value(lngIndex, 10) = oALLOC.AllocatedQty & "/" & oALLOC.AllocatedQtySS
        Else
            XA.Value(lngIndex, 7) = oALLOC.OrderedQty
            XA.Value(lngIndex, 8) = oALLOC.DeliveredSoFar
            XA.Value(lngIndex, 10) = oALLOC.AllocatedQty
        End If
        XA.Value(lngIndex, 9) = IIf(strWSName <> oALLOC.WSLock, "**Locked by other workstation:" & oALLOC.WSLock, oALLOC.OrderDetails)
        ...
        If strWSName <> oALLOC.WSLock Then
            XA.Value(lngIndex, 11) = 0
        Else
            XA.Value(lngIndex, 11) = IIf((FNN(oALLOC.AllocatedQty) + FNN(oALLOC.AllocatedQtySS)) > 0, -1, 0)
        End If
        XA.Value(lngIndex, 12) = IIf(oALLOC.UsesSubstitutions = "Y", "Y", "")
        XA.Value(lngIndex, 13) = oALLOC.COLID
        XA.Value(lngIndex, 14) = ""
        XA.Value(lngIndex, 15) = oALLOC.QtyOnHand
        XA.Value(lngIndex, 16) = oALLOC.Key
     '   XA.Value(lngIndex, 18) = ""
        XA.Value(lngIndex, 19) = IIf(oALLOC.QtyonCO > XA.Value(lngIndex, 7), 1, 0)
        XA.Value(lngIndex, 20) = oALLOC.WSLock
        XA.Value(lngIndex, 24) = oALLOC.CustomerName
        XA.Value(lngIndex, 25) = oALLOC.CustomerBlocked
        XA.Value(lngIndex, 26) = oALLOC.Status
...
Private Sub Form_Load()
    ...
    bOKtoClose = False
    lblWarning.Caption = "This list will exclude items that already appear on un-issued documents. Cancel any unwanted GDNs or Invoices"
    bSkipResize = False
...
Dim strErrPos As String
    ...
    str = FNS(XA.Value(Grid1.Bookmark, 18))
...
Dim strStatus As String
    ...
    If Left(XA(Bookmark, 9), 2) = "**" Then
        RowStyle.BackColor = COLOR_CANCELLED
        Exit Sub
    End If
    If InStr(1, XA(Bookmark, 9), "*") > 0 Then  'LOCKED
        RowStyle.ForeColor = vbRed
    End If
    strStatus = XA(Bookmark, 26)
...
'        RowStyle.BackColor = COLOUR_FULFIL_OK
        ...
        Grid1.Columns(8).BackColor = COLOUR_FULFIL_OK
        Grid1.Columns(10).BackColor = COLOUR_FULFIL_OK
        Grid1.Columns(9).BackColor = vbWhite
        Grid1.Columns(10).BackColor = vbWhite
        Grid1.Columns(3).BackColor = vbWhite
    ...
    End If
    If XA(Bookmark, 25) = True Then  'CUstomer is blocked
        RowStyle.BackColor = COLOR_CANCELLED
    End If
    If XA(Bookmark, 4) = False Then   'For existing pre-delivery invoice
        RowStyle.ForeColor = COLOUR_FULFIL_PREDELIVERY
    End If
    Grid1.Columns(9).BackColor = vbWhite
    Exit Sub
errHandler:
...
Dim tmp As Long
    ...
    If Left(Grid1.Columns(8).Value, 2) = "**" Then
        Cancel = True
        Exit Sub
    End If
    i = ColIndex + 1
    ...
    Case 10
        If oPC.AllowsSSInvoicing Then
            If Grid1.text > "" Then
                ...
                If bOK = True Then
                    Set oALLOC = cALLOC(XA(Grid1.Bookmark, 16))
                    oALLOC.BeginEdit
                    ...
                    If Not rs.eof Then
                        ...
                        rs.Update
                    End If
                    If (FNN(oALLOC.AllocatedQty) + FNN(oALLOC.AllocatedQtySS)) > TotalSSFIRM(XA(Grid1.Bookmark, 7), lngFirm, lngSS) - val(XA(Grid1.Bookmark, 8)) Then
                        MarkRowsValid 4, cALLOC(val(XA(Grid1.Bookmark, 16))).Key    'toomuch
                    ElseIf (FNN(oALLOC.AllocatedQty) + FNN(oALLOC.AllocatedQtySS)) > val(XA(Grid1.Bookmark, 15)) Then
                        MarkRowsValid 3, cALLOC(val(XA(Grid1.Bookmark, 16))).Key    'invalid
                    ElseIf (FNN(oALLOC.AllocatedQty) + FNN(oALLOC.AllocatedQtySS)) < TotalSSFIRM(XA(Grid1.Bookmark, 7), lngFirm, lngSS) - val(XA(Grid1.Bookmark, 8)) Then
                        MarkRowsValid 2, cALLOC(val(XA(Grid1.Bookmark, 16))).Key    'more
                    ElseIf (FNN(oALLOC.AllocatedQty) + FNN(oALLOC.AllocatedQtySS)) = TotalSSFIRM(XA(Grid1.Bookmark, 7), lngFirm, lngSS) - val(XA(Grid1.Bookmark, 8)) Then
                        MarkRowsValid 1, cALLOC(val(XA(Grid1.Bookmark, 16))).Key    'OK
                    End If
                    oALLOC.SetACtionYN (FNN(oALLOC.AllocatedQty) + FNN(oALLOC.AllocatedQtySS)) > 0
                    XA.Value(Grid1.Bookmark, 11) = IIf((FNN(oALLOC.AllocatedQty) + FNN(oALLOC.AllocatedQtySS)) > 0, -1, 0)
        ...
        Else
            If ConvertToLng(Grid1.text, lngResult) Then
                ...
                If Not rs.eof Then
                    ...
                    rs.Update
                End If
                If lngResult > val(XA(Grid1.Bookmark, 7)) - val(XA(Grid1.Bookmark, 8)) Then
                    MarkRowsValid 4, cALLOC(val(XA(Grid1.Bookmark, 16))).Key
                ElseIf lngResult > val(XA(Grid1.Bookmark, 15)) Then
                    MarkRowsValid 3, cALLOC(val(XA(Grid1.Bookmark, 16))).Key
                ElseIf lngResult < val(XA(Grid1.Bookmark, 7)) - val(XA(Grid1.Bookmark, 8)) Then
                    MarkRowsValid 2, cALLOC(val(XA(Grid1.Bookmark, 16))).Key
                ElseIf lngResult = val(XA(Grid1.Bookmark, 7)) - val(XA(Grid1.Bookmark, 8)) Then
                    MarkRowsValid 1, cALLOC(val(XA(Grid1.Bookmark, 16))).Key
                End If
                Grid1.Update
                cmdSave.Enabled = True
