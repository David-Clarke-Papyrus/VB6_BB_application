...
Object = "{DA4E6F7B-F5EE-43C5-A9A1-6BCC726F901E}#1.8#0"; "STATUS~1.OCX"
...
Begin VB.Form frmPOSMain 
   ...
   Begin TrueOleDBGrid60.TDBGrid G4 
      Height          =   2070
      Left            =   30
      OleObjectBlob   =   "frmPOSMain8.frx":0396
      TabIndex        =   4
      TabStop         =   0   'False
      Top             =   90
      Visible         =   0   'False
   ...
   Begin TrueOleDBGrid60.TDBGrid G1 
      Height          =   2970
      Left            =   60
      OleObjectBlob   =   "frmPOSMain8.frx":5ED5
      TabIndex        =   0
      TabStop         =   0   'False
      Top             =   90
      Width           =   11415
   ...
   Begin TrueOleDBGrid60.TDBGrid G2 
      Height          =   1380
      Left            =   0
      OleObjectBlob   =   "frmPOSMain8.frx":B064
      TabIndex        =   3
      TabStop         =   0   'False
      Top             =   3660
      Width           =   4485
   ...
   Begin TrueOleDBGrid60.TDBGrid G5 
      Height          =   2010
      Left            =   1950
      OleObjectBlob   =   "frmPOSMain8.frx":E6EF
      TabIndex        =   16
      TabStop         =   0   'False
      Top             =   855
      Visible         =   0   'False
   ...
   Begin TrueOleDBGrid60.TDBGrid G3 
      Height          =   2400
      Left            =   1500
      OleObjectBlob   =   "frmPOSMain8.frx":1299A
      TabIndex        =   17
      TabStop         =   0   'False
      Top             =   765
      Visible         =   0   'False
      Width           =   8745
   End
   Begin OposPOSPrinter_1_11_LibCtl.OPOSPOSPrinter OPOSPOSPrinter1 
      Left            =   9840
      OleObjectBlob   =   "frmPOSMain8.frx":17FE5
      Top             =   6240
   End
   Begin OposCashDrawer_1_11_LibCtl.OPOSCashDrawer OPOSCashDrawer1 
      Left            =   4380
      OleObjectBlob   =   "frmPOSMain8.frx":18009
      Top             =   4500
   End
   ...
   Begin VB.Menu mnuActions 
      Caption         =   "&Actions"
      Begin VB.Menu mnuLoadFromQuotation 
         Caption         =   "&Load sale from quotation"
      End
   End
End
...
Dim Res As Boolean
    Res = False
    Do Until Res = True
        If Not SecurityControl(eOperator, lngStaffID, strName, bCancelled, "Enter your signature.", "Your signature is invalid") Then
    ...
    Select Case enPresentState
            ...
            If bCustomerVisible = True Then
                If G3.Visible = True Then   '  The customers orders are being displayed
                    If oExchange.BalanceOwing < 0 Then
                        Stat "Scan or (X)Cancel trans.,(C)Cash refund,(CC)Reverse credit card,(CV)Credit voucher,(Dn)Del sale,(DPn)Del paymt."
                    Else
                       ' Stat "Scan or (X)Cancel trans.,(C)Cash,(V)Voucher,(CC)Card,(CV)Credit voucher,(Q)Cheque,(AC)On account,(Dn)Del prod,(DPn)Del paymt.,(DDP) Direct deposit"
                        Stat "Scan or (X)Cancel trans.,(C)Cash,(V)Voucher,(CC)Card,(Q)Cheque,(AC)On account,(Dn)Del prod,(DPn)Del paymt.,(DDP) Direct deposit"
                    End If
                Else
                    Stat "Scan or (X)Cancel trans.,(C)Cash,(V)Voucher,(CC)Card,(Q)Cheque,(AC)On account,(Dn)Del sale,(DPn)Del paymt.,(DDP) Direct deposit"
                End If
            Else
                If oExchange.BalanceOwing < 0 Then
                ...
                Else
                 '   Stat "Scan or (X)Cancel trans.,(C)Cash,(V)Voucher,(CC)Card,(CV)Credit voucher,(Q)Cheque,(AC)On account,(Dn)Del sale,(DPn)Del paymt.,(FC) Find customer"
        ...
        Case eStart
           ...
           ' Stat "Scan or (X)Cancel trans.,(C)Cash,(V)Voucher,(CC)Card,(CN)Credit note,(Q)Cheque,(AC)Account,(Dn)Del sale,(DPn)Del paymt.,(FC) Find customer"
            Stat "Start by scan or (A)Appro,(AR)Appro return,(Vn)Void,(OR)Place order,(RDEP)Refund deposit, (PA) Pay account, (PC)Petty cash,(PCR)Petty cash return, (CNA) Credit note (A/c cust)"
            ClearCustomer
            ClearTextFields
            SetForCOLSVisible False
            AutoSelect txtInput
...
Private Sub oExchange_GetCustomer()
    If Not oExchange.Customer.ID > 0 Then
        GetCustomer
    End If
End Sub
...
Private Sub GetCustomerFromMasterDB_control()
...
strPos = "Pos 2"
    ...
    strPos = "pOS 2.001 "
    strPos = "pOS 2.01 " & (OPOSPOSPrinter1 Is Nothing)
        With OPOSPOSPrinter1
    strPos = "pOS 2.02 " & (OPOSPOSPrinter1 Is Nothing)
  '  MsgBox "CONVERTtOtYPE =" & ConvertToType(oExchange.transactionType)
...
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmPOSMain.PrintSalesSlip(pCopyCount,bReprint)", Array(pCopyCount, bReprint), , , "strPos", Array(strPos)
End Sub
...
''    ElseIf Action_SearchCustomer("CN") Then
    ...
    Else
        ...
        End If
'        If Not bCustomerVisible Then
'            If Action_SearchCustomer("CN") Then
''''            If Not GetCustomer Then
''''                Action_PaymentType_DirectDeposit = eSale
''''                Exit Function
'            End If
...
Private Function Action_PettyCash() As eState
    ...
    If frmPC.Cancelled Then
        ...
        setInputBox "", "", "", True
    Else
        If SecurityControl(eOperator, lngOPID, strName, , "Enter your security key.", "Your key is invalid") Then
...
Private Function Action_Price() As eState
    ...
    If strPrefix = ".." Then
        ...
        If oExchange.LoyaltyValue > 0 Then
            lblCustomername.Caption = DisplayCustomerDetails
        End If
    ElseIf bShiftDown And oSALELine.IsDiscountAllowed Then
...
Dim frm As frmOD
    On Error GoTo errHandler
    If SecurityControl(eOperator, lngStaffID, strName, , "Enter security code to open drawer") Then
        Set frm = New frmOD
        frm.Show vbModal
        If frm.Cancelled Then
            Unload frm
              ...
              '  If X4(X4.Find(1, 1, iToVoid, XORDER_ASCEND, XCOMP_EQ, XTYPE_LONG), 7) = "DEPOSIT" Then
                ...
                Else
                    ...
                    Do Until Res = True Or bCancelled = True
                        If Not SecurityControl(eSupervisor, lngStaffID, strName, bCancelled, "Enter your signature.", "Your signature is invalid for this action") Then
                            Res = False
                        Else
                            Res = True
                        End If
