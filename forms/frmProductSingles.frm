...
Begin VB.Form frmProductSingles 
   ...
   Begin VB.TextBox txtSPFactor 
      ...
      Width           =   885
   End
   Begin VB.CommandButton cmdUnlockPrice 
      Height          =   345
      Left            =   2370
      Picture         =   "frmProductSingles.frx":030A
      Style           =   1  'Graphical
      TabIndex        =   55
      Top             =   5370
      Width           =   540
   ...
   Begin VB.ComboBox cboSearch 
      ...
      Height          =   360
      Index           =   0
      ItemData        =   "frmProductSingles.frx":0694
      Left            =   6360
      List            =   "frmProductSingles.frx":0696
      TabIndex        =   11
      Top             =   1830
      Width           =   2940
   End
   Begin VB.ComboBox cboSearch 
      ...
      Height          =   360
      Index           =   1
      ItemData        =   "frmProductSingles.frx":0698
      Left            =   6360
      List            =   "frmProductSingles.frx":069A
      TabIndex        =   12
      Top             =   2190
      Width           =   2940
   End
   Begin VB.ComboBox cboSearch 
      ...
      Height          =   360
      Index           =   2
      ItemData        =   "frmProductSingles.frx":069C
      Left            =   6360
      List            =   "frmProductSingles.frx":069E
      TabIndex        =   13
      Top             =   2550
      Width           =   2940
   End
   Begin VB.ComboBox cboSearch 
      ...
      Height          =   360
      Index           =   3
      ItemData        =   "frmProductSingles.frx":06A0
      Left            =   6360
      List            =   "frmProductSingles.frx":06A2
      TabIndex        =   14
      Top             =   2925
      Width           =   2940
   End
   Begin VB.ComboBox cboSearch 
      ...
      Height          =   360
      Index           =   4
      ItemData        =   "frmProductSingles.frx":06A4
      Left            =   6360
      List            =   "frmProductSingles.frx":06A6
      TabIndex        =   15
      Top             =   3285
      Width           =   2940
   End
   Begin VB.ComboBox cboSearch 
      ...
      Height          =   360
      Index           =   5
      ItemData        =   "frmProductSingles.frx":06A8
      Left            =   6360
      List            =   "frmProductSingles.frx":06AA
      TabIndex        =   16
      Top             =   3645
      Width           =   2940
   End
   Begin VB.ComboBox cboSearch 
      ...
      Height          =   360
      Index           =   6
      ItemData        =   "frmProductSingles.frx":06AC
      Left            =   6360
      List            =   "frmProductSingles.frx":06AE
      TabIndex        =   17
      Top             =   4005
      Width           =   2940
   End
   Begin VB.ComboBox cboSearch 
      ...
      Height          =   360
      Index           =   7
      ItemData        =   "frmProductSingles.frx":06B0
      Left            =   6360
      List            =   "frmProductSingles.frx":06B2
      TabIndex        =   18
      Top             =   4365
      Width           =   2940
   End
   ...
   Begin VB.CommandButton cmdCancel 
      ...
      Height          =   615
      Left            =   7275
      Picture         =   "frmProductSingles.frx":0A3E
      Style           =   1  'Graphical
      TabIndex        =   20
      Top             =   7065
      Width           =   1000
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.TextBox Text1 
         ...
         TabIndex        =   39
         TabStop         =   0   'False
         Text            =   "frmProductSingles.frx":0DC8
         Top             =   390
         Width           =   5415
      End
...
Dim fs As New FileSystemObject
    ...
    SaveSetting "PBKS", "Folders", "Images", fs.GetParentFolderName(strFilePath)
    AddImageToDB strFilePath, oProd.PID
    If fs.FileExists(strFilePath) Then
        oProd.SetBIC fs.GetFileName(strFilePath)
        LoadPictureFromFile strFilePath
    End If
'    If fs.FileExists(Replace(strFilePath, "zoom_", "thumb_")) Then
'        bytTemp = FileToByteArray(Replace(strFilePath, "zoom_", "thumb_"))
'        oProd.SmallImage = bytTemp
...
errHandler:
    ...
    HandleError
End Sub
Private Sub LoadPictureFromFile(strFilePath As String)
    On Error GoTo errHandler
Dim bytTemp() As Byte
    bytTemp = FileToByteArray(strFilePath)
...
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmProductSingles.LoadPictureFromDB(bytTemp)", bytTemp
End Sub

Private Sub cmdUnlockPrice_Click()
Dim bIsSUpervisor As Boolean
Dim strName As String
    If SecurityControl(enSECURITY_CUSTDISCOUNT_AUTH, bCancelled, "Enter your signature", "You do not have permission to set the price (or your signature is invalid).", bIsSUpervisor, strName, lngSMIDPriceChange) = True Then
        txtSP.Enabled = True
        txtSPFactor.Enabled = True
...
Private Sub txtSP_Change()
    If flgLoading Then Exit Sub
    If IsNumeric(txtSP) And dblSQM > 0 Then
        flgLoading = True
        mSPFactor = CDbl(txtSP) / dblSQM
        txtSPFactor = Format(mSPFactor, "\R###,##0")
        flgLoading = False
    End If
End Sub

Private Sub txtSPFactor_Change()
    If flgLoading Then Exit Sub
    If IsNumeric(txtSPFactor) Then
        mSPFactor = CDbl(txtSPFactor)
        txtSPFactor.ForeColor = &H8000000D
        oProd.SetSP CStr(mSPFactor * dblSQM)
        flgLoading = True
        txtSP = Format(mSPFactor * dblSQM, "\R###,##0")
        flgLoading = False
    Else
        txtSPFactor.ForeColor = vbRed
    End If
End Sub

Private Sub txtSPFactor_GotFocus()
    txtSPFactor = CStr(CLng(mSPFactor))
    AutoSelect txtSPFactor
End Sub

Private Sub txtSPFactor_LostFocus()
    txtSPFactor = CStr(mSPFactor)
End Sub

Private Sub txtSPFactor_Validate(Cancel As Boolean)
    Cancel = Not (IsNumeric(txtSPFactor))
End Sub
...
End Sub
Private Sub LoadSqM()
    dblSQM = (oProd.Length * oProd.Width) / 1000000
    lblSqM.Caption = Format(dblSQM, "##.##")
...
Dim fs As New FileSystemObject
    ...
    txtWidth = oProd.WidthF
    LoadSqM
    lblImageFilename.Caption = oProd.BICDescription
    If IsNumeric(txtSP) And dblSQM > 0 Then
        txtSPFactor = Format(CDbl(txtSP) / dblSQM, "\R###,##0")
    End If
    ...
    Case Else
        optIP.Value = True
    End Select
    If oProd.ProductSections.Count > 0 Then Me.cboSection = oProd.ProductSections(1).Description
    If oProd.PID > "" Then
        strFilePath = oPC.SharedFolderRoot & "\Images\" & oProd.BIC
        If fs.FileExists(strFilePath) Then
            LoadPictureFromFile (strFilePath)
        End If
    End If
...
Private Sub txtSP_Validate(Cancel As Boolean)
        On Error Resume Next
    If flgLoading Then Exit Sub
    If Not oProd.SetSP(txtSP, , lngSMIDPriceChange) Then
        Cancel = True
    End If
    txtSP = oProd.SPF
    Exit Sub
...
Private Sub txtCost_Validate(Cancel As Boolean)
        On Error Resume Next
    If flgLoading Then Exit Sub
    If Not oProd.SetRRP(txtCost) Then
        Cancel = True
    End If
    txtCost = oProd.RRPDecF
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
