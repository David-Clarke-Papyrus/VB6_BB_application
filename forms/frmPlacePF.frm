...
Begin VB.Form frmPlacePF 
   ...
   Begin VB.CommandButton cmdSearch 
      ...
      Width           =   2925
   End
   Begin VB.CommandButton cmdPlacePF 
      BackColor       =   &H00C4BCA4&
      Caption         =   "&Create pro-forma invoice"
      Enabled         =   0   'False
      Height          =   825
      Left            =   6375
      Picture         =   "frmPlacePF.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   8
      Top             =   4425
      Width           =   1470
   ...
   Begin TrueOleDBGrid60.TDBGrid G1 
      Height          =   1440
      Left            =   150
      OleObjectBlob   =   "frmPlacePF.frx":0714
      TabIndex        =   6
      Top             =   2760
      Width           =   9390
   End
...
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmPlacePF.mnuSaveLayout"
End Sub
...
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmPlacePF.SetMenu"
End Sub


Private Sub cmdPlacePF_Click()
    On Error GoTo errHandler
    PlacePF strType
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmPlacePF.cmdPlacePF_Click", , EA_NORERAISE
    HandleError
End Sub
...
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmPlacePF.Form_Deactivate", , EA_NORERAISE
    HandleError
End Sub
...
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmPlacePF.G1_ColResize(ColIndex,Cancel)", Array(ColIndex, Cancel), EA_NORERAISE
    HandleError
End Sub
...
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.oCust_PossibleDuplicates(pDuplicates)", pDuplicates
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmPlacePF.oCust_PossibleDuplicates(pDuplicates)", pDuplicates, EA_NORERAISE
    HandleError
End Sub
...
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.ShowDuplicates(pDuplicates)", pDuplicates
    Exit Function
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmPlacePF.ShowDuplicates(pDuplicates)", pDuplicates
...
Dim j As Long
   ...
   ' Set XA = p
    ...
    If UCase(strType) = "INVOICE" Then
        ...
        cmdPlacePF.Caption = "&Create invoice"
    ElseIf UCase(strType) = "PF" Then
        Me.Caption = "Place pro-forma invoice"
        cmdPlacePF.Caption = "&Create pro-forma invoice"
    End If
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.Component(p,pType)", Array(p, pType)
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmPlacePF.component(p,pType)", Array(p, pType)
...
'    frmPlacePF.Enabled = (lngTPID <> 0) And XA.Count(1) > 0
'    Exit Sub
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.cmdClearBC_Click", , EA_NORERAISE
'    HandleError
'End Sub
...
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.cmdClose_Click", , EA_NORERAISE
'    HandleError
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ...
    HandleError
End Sub
Private Sub frmPlacePF_Click()
    On Error GoTo errHandler
    G1.Update
        PlacePF strType
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.frmPlacePF_Click", , EA_NORERAISE
'    HandleError
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
...
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.PlaceOnReserve"
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmPlacePF.PlaceOnReserve"
End Sub
Private Sub PlacePF(strType As String)
    On Error GoTo errHandler
Dim i As Integer
Dim oILine As a_InvoiceLine
Dim lngResult As Long
...
Dim bIssue As Boolean
    ...
    If bProductToInvoice = False Then
        ...
        Exit Sub
    End If
    If MsgBox("You are placing " & IIf(strType = "PF", "a pro-forma invoice", "an invoice") & " for " & txtInitials & " " & txtName & "?", vbQuestion + vbOKCancel, "Confirm") = vbCancel Then
        Exit Sub
    End If
    If (oPC.GetProperty("IssueQuickPFs") = "TRUE" And strType = "PF") Or (oPC.GetProperty("IssueQuickInvoices") = "TRUE" And strType <> "PF") Then
        If oPC.Configuration.SignTransactions = True Then
            ...
            End If
        Else
            If oPF.Status = stInProcess Then
    ...
    Set oPF = Nothing
    Set oPF = New a_Invoice
    oPF.BeginEdit
    oPF.SetCustomer lngTPID
   ' oPF.setMemo FNS(txtAddress)
    oPF.VATable = True
    oPF.StaffID = gSTAFFID

    For i = 1 To XA.UpperBound(1)
        Set oILine = oPF.InvoiceLines.Add
        oILine.BeginEdit
        oILine.PID = XA(i, 8)
       oILine.Price = XA(i, 9)
        oILine.VATRate = oPC.Configuration.VATRate
      '  oILine.code = XA(i, 7)
        oILine.SetQty XA(i, 4)
      '  oILine.SetDeposit XA(i, 5)
        oILine.SetRef XA(i, 6)
        oILine.ApplyEdit
    Next
    If bIssue Then
        oPF.SetStatus stCOMPLETE
    Else
        oPF.SetStatus stInProcess
    End If
    If strType = "PF" Then oPF.SetProforma
    oPF.Post
    
    Screen.MousePointer = vbDefault
    MsgBox IIf(strType = "PF", "Pro-forma issued", "Invoice issued"), , "Status"
Dim f As New frmInvoicePreview
    f.component oPF.InvoiceID
    f.Show
    Unload Me
    
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.cmdPlacePF_Click"
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmPlacePF.PlacePF(strType)", strType
End Sub
...
Dim frmC As frmBrowseCustomers2
    ...
    Set frmC = Nothing
    lblFound.Visible = (lngTPID <> 0)
    cmdPlacePF.Enabled = (lngTPID <> 0) And XA.Count(1) > 0
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.cmdSearch_Click", , EA_NORERAISE
'    HandleError
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
...
Private Sub Form_Activate()
    On Error GoTo errHandler
    cmdPlacePF.Enabled = XA.Count(1) > 0 And lngTPID > 0 And lblFound.Visible = True
    SetMenu
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.Form_Activate", , EA_NORERAISE
'    HandleError
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
...
Private Sub Form_Load()
    ...
    Me.cmdPlacePF.Caption = "Place pro-forma"
    Me.Caption = "Quick pro-forma"
    Me.Height = 6200
    Me.Width = 10000
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.Form_Load", , EA_NORERAISE
'    HandleError
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
...
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.cboBC_SelectionChanged", , EA_NORERAISE
'    HandleError
'End Sub
...
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.Form_Unload(Cancel)", Cancel, EA_NORERAISE
'    HandleError
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
...
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.Label2_Click", , EA_NORERAISE
'    HandleError
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
...
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.Text1_Change", , EA_NORERAISE
'    HandleError
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
...
Private Sub G1_AfterDelete()
    On Error GoTo errHandler
    If XA.Count(1) = 0 Then Me.cmdPlacePF.Enabled = False
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.G1_AfterDelete", , EA_NORERAISE
'    HandleError
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
...
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.G1_BeforeColEdit(ColIndex,KeyAscii,Cancel)", Array(ColIndex, KeyAscii, _
'         Cancel), EA_NORERAISE
'    HandleError
    Exit Sub
errHandler:
...
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmPlacePF.G1_BeforeColUpdate(ColIndex,OldValue,Cancel)", Array(ColIndex, OldValue, _
         Cancel), EA_NORERAISE
    HandleError
End Sub
...
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.G1_KeyPress(KeyAscii)", KeyAscii, EA_NORERAISE
'    HandleError
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
...
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.G1_LostFocus", , EA_NORERAISE
'    HandleError
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
...
Dim i As Integer
    ...
    Next
    G1.ReBind
    cmdPlacePF.Enabled = Len(txtPhone) > 5 And Len(txtName) > 1 And XA.Count(1) > 0
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.txtName_Change", , EA_NORERAISE
'    HandleError
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
...
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.txtPhone_Change", , EA_NORERAISE
'    HandleError
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
...
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmPlacePF.txtPhone_Validate(Cancel)", Cancel, EA_NORERAISE
'    HandleError
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
