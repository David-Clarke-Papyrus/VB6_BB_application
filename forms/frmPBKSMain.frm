...
Begin VB.MDIForm frmMain 
   ...
   Begin VB.Menu mnuFile 
      ...
      Begin VB.Menu mnuNewTestFromLive 
         Caption         =   "Create new test database from live database (server only)"
      End
      Begin VB.Menu mnuSepFile_1 
         Caption         =   "-"
      End
   ...
   Begin VB.Menu mnuTr 
      ...
      Begin VB.Menu mnuStatements 
         Caption         =   "Debtors"
         Begin VB.Menu mnuAgedBalances 
            Caption         =   "Aged balances"
         End
         ...
         End
         Begin VB.Menu mnuChangePeriods 
   ...
   Begin VB.Menu mnuBrowse 
      ...
      Begin VB.Menu mnuBrowseBooks 
         ...
         Shortcut        =   ^F
      End
      Begin VB.Menu mnuBrowseAQBooks 
         Caption         =   "&Antiquarian books"
      End
      ...
      Begin VB.Menu hx 
         Caption         =   "-"
      End
      Begin VB.Menu mnuBrowseDBJNLS 
         Caption         =   "Debtors' journals"
      End
      Begin VB.Menu mnubrowsePayments 
         Caption         =   "Pay&ments"
      End
   ...
   Begin VB.Menu mnuNew 
      ...
      Begin VB.Menu mnuNewCustomer 
         Caption         =   "&Customer (private)"
      End
      Begin VB.Menu mnuNewCustomer_Bus 
         Caption         =   "&Customer (business)"
      End
      Begin VB.Menu mnuNewCustomer_BC 
         Caption         =   "&Customer (book club)"
      End
      Begin VB.Menu mnuNewLoyalty 
         Caption         =   "&Loyalty club member"
         Shortcut        =   ^L
      End
      ...
      End
      Begin VB.Menu mnuNewAQBook 
         Caption         =   "Antiquarian book"
      End
      ...
      Begin VB.Menu h4b 
         Caption         =   "-"
      End
      Begin VB.Menu mnuDebtorJournal 
         Caption         =   "Debtors journals"
      End
      Begin VB.Menu mnuDebtorsPayment 
         Caption         =   "Debtors payment"
      End
      Begin VB.Menu h4bb 
         Caption         =   "-"
      End
   ...
   Begin VB.Menu mnuMast 
      ...
      End
      Begin VB.Menu mnuProductCategorisations 
   ...
   Begin VB.Menu mnuOF 
      ...
      Begin VB.Menu mnuInvoicing 
         Caption         =   "Invoicing"
         Begin VB.Menu mnuGDNsToInvoice 
            Caption         =   "Deliveries to invoice (excluding customers with completeness requirement)"
         End
   ...
   Begin VB.Menu mnuCashbook 
      Caption         =   "&Cashbook"
      Visible         =   0   'False
      Begin VB.Menu mnuImportBankfile 
         Caption         =   "Import file from bank"
      End
      Begin VB.Menu mnuBrowseBankPostings 
         Caption         =   "Browse bank postings"
      End
   End
   Begin VB.Menu mnuTools 
      ...
      Begin VB.Menu mnuIMport 
         ...
         End
         Begin VB.Menu mnuIMportGRNTab 
            Caption         =   "Import GRN from tab delimited file"
         End
      End
   ...
   Begin VB.Menu mnuBrowseInvoicesPopup 
      ...
      Visible         =   0   'False
      Begin VB.Menu mnuBrowseInvoicesPopup_Print 
         Caption         =   "Print"
      End
   End
...
Private Sub mnuProductCategorisations_Click()
Dim f As New frmProductCategorizations
    f.Show
...
Private Sub NewCustomer(pType As enumCustomerType)
    On Error GoTo errHandler
...
End Sub
Private Sub NewLoyaltyCustomer()
    On Error GoTo errHandler
...
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
    ErrorIn "frmMain.BrowseInvoices"
End Sub
...
End Sub
Private Sub NewInvoice(Proforma As Boolean)
    On Error GoTo errHandler
Dim lngTPID As Long
Dim frm As frmBrowseCustomers2
Dim frmI As frmInvoice
    ...
    frmI.Component lngTPID, , Proforma
    frmI.Show
EXIT_Handler:
    Me.MousePointer = vbDefault
    Exit Sub
...
Dim s As String
    ...
    Me.mnuCashbook.Visible = fRunningInIde(Me.hwnd)
    ...
    Me.mnuPrintpickingSlip.Visible = oPC.UniqueProducts
    mnuReorderFromBrowse.Visible = True  'oPC.UniqueProducts
    mnuOrderFulfilmentAppros.Visible = oPC.CanGenerateApprosFromOrderFulfilment
    mnuProductCategorisations.Visible = oPC.UniqueProducts
    If Not fRunningInIde(Me.hwnd) Then
...
restart:
        ...
        rs.Open "SELECT LNG_LastKnownGood FROM tLastKnownGood WHERE LNG_Databasename = 'PBKS'", oPC.COShort, adOpenStatic
        ...
        lkgPBKSDate = rs.Fields(0)
        rs.Close
        If DateDiff("d", lkgMasterDate, Date) > 1 Or DateDiff("d", lkgPBKSDate, Date) > 1 Then
            If cntTest = 0 Then
                cntTest = cntTest + 1
                ...
                oPC.COShort.Execute " DBCC CHECKDB ('PBKS') WITH NO_INFOMSGS;"
                oPC.COShort.Execute " DBCC CHECKDB ('Master') WITH NO_INFOMSGS;"
                oPC.COShort.Execute "dbo.UpdateLNG"
    ...
    End If
    'Setting up system error handler
    errSysHandlerSet
...
Private Sub PaintFirstScreen()
    ...
    Case 3    'Antiquarian only
        ...
        mnuBrowseAQBooks.Visible = True
    End Select
   ' mnuNewLoyalty.Visible = oPC.Configuration.SupportsLoyaltyClub
...
Dim f As frmCashUPForBlind
    ...
    If SecurityControl(enSECURITY_CASHUP_SIGN, , "Opening cash-up forms", "You do not have permission to open the cash-up forms (or your signature is invalid).") = False Then Exit Sub
...
Private Sub mnuConfig_Click()
    ...
    If SecurityControl(enSECURITY_CONFIG_SIGN, , "Entering configuration", "You do not have permission to open the configuration form (or your signature is invalid).") = False Then Exit Sub
    Set frmConfiguration = New frmConfiguration
...
'        Exit Sub
'    End If
    If SecurityControl(enSECURITY_DICT_SIGN, , "Enter your security code.", "You do not have permission to open the dictionary (or your signature is invalid).") = False Then Exit Sub
...
errHandler:
    ...
    HandleError
End Sub
Private Sub mnuImportGRNTab_Click()
    On Error GoTo errHandler
Dim frm1 As frmdelBB
Dim lngTPID As Long
Dim frm As frmBrowseSUppliers2
Dim bCancel As Boolean
...
'    If lngTPID = 0 Then Exit Sub
    ...
    CD1.DialogTitle = "Find tab delimited file containing GRN details"
    CD1.DefaultExt = ".txt"
    CD1.InitDir = "C:\PBKS"
    CD1.ShowOpen
    strFilename = CD1.FileName
    
  '  oSM.CreateGRNFromFile lngTPID, strFilename, NewID
    
    Set oGRN = New a_Delivery
    If lngTRID > 0 Then
        oGRN.Load lngTRID
        Set frm1 = New frmdelBB
        frm1.Component False, , oGRN
        frm1.Show
    End If
...
Private Sub mnuOrderRequests_Click()
    On Error GoTo errHandler
Dim Fr As New frmOrderRequests
    Fr.Show
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
...
Private Sub TBHead_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Case "BGEN"
        BrowseGenStock
    Case "BBKS"
        If oPC.AllowAntiquarionSearch = 3 Then
            BrowseAntBooks
        Else
            BrowseBooks
        End If
...
Private Sub mnuSetDeal_Click()
    On Error GoTo errHandler
    If Me.ActiveForm Is frmREORDER_SAL Then
        frmREORDER_SAL.SetDeal
    Else
        frmREORDER_CUST.SetDeal
    End If
    Exit Sub
errHandler:
    If ErrMustStop Then Debug.Assert False: Resume
...
errHandler:
    ...
    HandleError
End Sub
'Private Sub mnuBrowseInvoicesPopup_Print()
'    Me.ActiveForm.PrintSet
'End Sub
...
Public Sub PastelinestoNEWInvoice(Optional Proforma As String = "")
    On Error GoTo errHandler
Dim rs As ADODB.Recordset
Dim lngQUID As Long
Dim oSM As New z_StockManager
...
Dim qty As Long
    ...
    lngOrderID = oSM.CreateNewInvoice(lngTPID, Proforma)
    ...
    oInv.BeginEdit
    If (Proforma = "PF") Then oInv.SetProforma
    oInv.ApplyEdit
    Set rs = oPC.LinesClipboard
    If rs.State <> 0 Then
