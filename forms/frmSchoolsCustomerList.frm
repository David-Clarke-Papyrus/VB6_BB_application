...
Begin VB.Form frmSchoolsCustomerList 
   Caption         =   "Schools' customers"
   ClientHeight    =   4200
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6105
   ...
   Begin VB.CommandButton cmdDeleteSchool 
      BackColor       =   &H00C4BCA4&
   ...
   Begin TrueOleDBGrid60.TDBGrid G 
      Bindings        =   "frmSchoolsCustomerList.frx":0000
      Height          =   2280
      Left            =   210
      OleObjectBlob   =   "frmSchoolsCustomerList.frx":0015
      TabIndex        =   0
      Top             =   990
      Width           =   5430
   End
   ...
   End
End
Attribute VB_Name = "frmSchoolsCustomerList"
...
Private Sub cmdDeleteSchool_Click()
    If MsgBox("You are going to delete all entries for school: " & cboSchoolCode & ". Continue?", vbQuestion + vbYesNo, "Confirm") = vbNo Then
        Exit Sub
    End If
    oPC.OpenDBSHort
    oPC.COShort.Execute "DELETE FROM tSchoolsCustomer WHERE SC_Schoolcode = '" & cboSchoolCode & "'"
    oPC.COShort.Execute "DELETE FROM tProductList WHERE PLI_LISTNAME = '" & cboSchoolCode & "'"
    oPC.COShort.Execute "DELETE tTmpInvoiceLine FROM tTmpInvoiceLine LEFT JOIN tSchoolsCustomer ON TIL_SC_ID = SC_ID WHERE SC_ID IS NULL"
...
Private Sub cmdImport_Click()
    If MsgBox("You want to delete any customers for school code " & cboSchoolCode & " and import a new list?", vbQuestion + vbYesNo, "Confirm") = vbNo Then
        Exit Sub
    End If
...
Private Sub cmdLoad_Click()
    ...
    Adodc1.RecordSource = "Select * FROM tSchoolsCustomer WHERE SC_SchoolCode = '" & cboSchoolCode & "' ORDER BY SC_SchoolCode,SC_ClassCode,SC_NAME"
...
Private Sub G_Error(ByVal DataError As Integer, Response As Integer)
    If DataError = 2601 Then
        MsgBox "There is already a customer name like this for this school code.", vbInformation, "Can't add this record"
        Response = 0
    End If
End Sub
...
Dim ar() As String
    ...
    Do While Not oTF.IsEOF
        ...
        If Not oTF.IsEOF Then
            ar() = Split(strLine, ",")
            oPC.COShort.Execute "INSERT INTO tSchoolsCustomer (SC_SchoolCode,SC_ClassCode,SC_Name) VALUES ('" & cboSchoolCode & "','" & ar(1) & "','" & ar(0) & "')"
        End If
    Loop
    oTF.CloseTextFile
    oPC.DisconnectDBShort
