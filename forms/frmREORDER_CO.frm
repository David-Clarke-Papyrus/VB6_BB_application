...
Dim dteTMP As Date
    ...
    If strType = "CUST" Then
        If oPC.Configuration.ReorderPerCOL Then
            rs.Open "SELECT * FROM dbo.tREORDERCUSTByCOL WHERE STATUS <> 'X' AND STAFFID = " & lngStaffID & " AND WSNAME = '" & oPC.NameOfPC & "'", oPC.COShort, adOpenKeyset, adLockOptimistic
        Else
            rs.Open "SELECT * FROM dbo.tREORDERGENERAL WHERE STATUS <> 'X'" & " AND WSNAME = '" & oPC.NameOfPC & "'", oPC.COShort, adOpenKeyset, adLockOptimistic
        End If
    Else
        rs.Open "SELECT * FROM dbo.tREORDERGENERAL WHERE STATUS <> 'X'" & " AND WSNAME = '" & oPC.NameOfPC & "'", oPC.COShort, adOpenKeyset, adLockOptimistic
    End If
...
Private Sub cboStaffMember_Validate(Cancel As Boolean)
    lngStaffID = oPC.Configuration.Staff.FindStaffByName(cboStaffMember).ID
    SaveSetting "PBKS", "USERS", "REORDER", lngStaffID
    LoadGrid
End Sub

'Private Sub chkFilter_Click()
'    On Error GoTo errHandler
'    bFilterPrint = (Me.chkFilter = 1)
'    Exit Sub
'errHandler:
'    If ErrMustStop Then Debug.Assert False: Resume
'    ErrorIn "frmREORDER_CO.chkFilter_Click", , EA_NORERAISE
'    HandleError
'End Sub
...
Dim i As Long
    ...
    Screen.MousePointer = vbHourglass
    oPC.COShort.Execute "EXEC DropTableIfExists tCREATEPOS_TEMP,''"
    oPC.COShort.Execute "CREATE TABLE tCREATEPOs_TEMP (PID CHAR(40),CODE CHAR(13),COLID INTEGER,REF CHAR(25),TPID INTEGER,DLID INTEGER,QTYFIRM INTEGER,QTYSS INTEGER,PRICE INTEGER)"
    For i = 1 To lngArrayRows
        If XA.Value(i, 11) > 0 Or XA.Value(i, 12) > 0 Then
            oPC.COShort.Execute "INSERT INTO tCREATEPOS_TEMP (PID,CODE,COLID,REF,TPID,DLID,QTYFIRM,QTYSS,PRICE) VALUES ('" & XA.Value(i, 21) & "','" & XA.Value(i, 1) & "'," & XA.Value(i, 26) & ",'" & XA.Value(i, 20) & "'," & XA.Value(i, 22) & "," & XA.Value(i, 23) & "," & XA.Value(i, 11) & "," & XA.Value(i, 12) & "," & XA.Value(i, 27) & ")"
        End If
    Next i
    PB1.Visible = True
   ...
   ' Set rs = Nothing
    If strType = "CUST" Then
        oPC.COShort.Execute "DELETE FROM tREORDERCUSTByCOL WHERE STAFFID = " & lngStaffID & " AND WSNAME = '" & oPC.NameOfPC & "'"
    Else
...
Dim strPID As String
    ...
    If strType = "CUST" Then
        oPC.COShort.Execute "UPDATE tREORDER1 SET Status = 'X' WHERE PID = '" & strPID & "'"
        oSM.MarkCOLsActionedForProduct strPID
    Else
        oPC.COShort.Execute "UPDATE tREORDER2 SET Status = 'X' WHERE PID = '" & strPID & "'"
...
Err_Execute:
   ...
   ' executing the query.
   If oPC.COShort.errors.Count > 0 Then
      For Each errLoop In oPC.COShort.errors
...
'-------------------------------
    ...
    If oPC.Configuration.ReorderPerCOL Then
        ...
        lngLastStaffID = GetSetting("PBKS", "USERS", "REORDER", "0")
